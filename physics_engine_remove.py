...
class BallEngine(Engine):
    ...
    def add_balls(self, center=(0., 0.), p_range=(-30, 30)):
        ...
        for i in range(self.n_ball):
            while True:
                ...
                flag = True
                for j in range(i):
                    if calc_dis([x, y], self.balls[j].position) < 20:
                        flag = False
                if flag:
                    break
    ...
    def add_impulse(self, p_range=(-80, 80)):
        for i in range(self.n_ball):
    ...
    def add_boundary_impulse(self, p_range=(-80, 80, -80, 80)):
        f_scale = 5e2
        eps = 2
        for i in range(self.n_ball):
            impulse = np.zeros(2)
    ...
    def change_rel(self):
        ...
        for i in range(self.n_ball):
            for j in range(i):
                rel.append([i, j])
        constraint_to_alter = np.random.choice(10, 2)
        for c_a in constraint_to_alter:
            a, b = rel[c_a]
            ball_a = self.balls[a]
            ball_b = self.balls[b]
            ca = ball_a.constraints
            cb = ball_b.constraints
            for c in ca:
                if c in cb:
                    # 1. remove constraint
                    ball_a._remove_constraint(c)
                    ball_b._remove_constraint(c)
                    self.space._remove_constraint(c)

                    # 2. sample new constraint
                    rel_type = rand_int(0, self.n_rel_type)
                    rel_type = 2
                    self.param[c_a, 0] = rel_type

                    pos_i = ball_a.position
                    pos_j = ball_b.position
                    ...
                    if rel_type == 0:
                        ...
                        # spring
                        rest_length = rand_float(20, 120)
                        self.param[c_a, 1] = rest_length
                        c = pymunk.DampedSpring(
                            ball_a, ball_b, (0, 0), (0, 0),
                            rest_length=rest_length, stiffness=20, damping=0.)
                        self.space.add(c)

                    elif rel_type == 2:
                        # string
                        rest_length = calc_dis(pos_i, pos_j)
                        self.param[c_a, 1] = rest_length
                        c = pymunk.SlideJoint(
                            ball_a, ball_b, (0, 0), (0, 0),
                            rest_length - 5, rest_length + 5)
    ...
    def criterion_allrel_y(self, t):
        # criterion if string is streched too long, break the link.
        rel_matrix = []
        for i in range(self.n_ball):
            for j in range(i):
                rel_matrix.append([i, j])
        constraint_to_alter = np.arange(10)
        max_y = 0
        for ball_id in range(self.n_ball):
            x, y = self.balls[ball_id].position
            #print(x, y, 'x y', max_y)
            if y > max_y:
                max_y = y
            if y > 55:
                # clear all constarints:
                #print('cleared', t)
                self.cleared = True
                for c_a in constraint_to_alter:
                    a, b = rel_matrix[c_a]
    ...
    def criterion_rel_string_length(self, t):
        # criterion if string is streched too long, break the link.
        rel_matrix = []
        for i in range(self.n_ball):
            for j in range(i):
        ...
        constraint_to_alter = np.arange(10)
        for c_a in constraint_to_alter:
            rel = self.param[c_a, 0]
            a, b = rel_matrix[c_a]
            ball_a = self.balls[a]
            ball_b = self.balls[b]
            pos_a = ball_a.position
            pos_b = ball_b.position
            dis = np.linalg.norm(pos_a - pos_b)
            # criterion if string is streched too long, break the link.
            if rel == 2:
                length = self.param[c_a, 1]
                print(dis - length)
                if (dis - length) > 15:
                    ca = ball_a.constraints
                    cb = ball_b.constraints
    ...
    def step(self, t, action=None):
        # print(self.balls[0].constraints, 'here!!!!!!!!!')
        self.state_prv = self.get_state()
        # self.add_action(action)
        self.add_boundary_impulse()
        self.space.step(self.dt)
        if not self.cleared:
            self.criterion_allrel_y(t)
        # if t > 0 and t % 100 == 0:
        #    self.change_rel()
        # self.change_speed()
        # print(self.balls[0].constraints, 'aaaaa!!!!!!!!!')
        if self.cleared and t < 20:
            return True
        elif not self.cleared and t > 67:
            return True
        else:
            return False

    def render(self, states, actions, rels, video=True, image=False, path=None, draw_edge=True,
